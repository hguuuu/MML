...
# use BERT embeddings
# pkl_file = '../embeddings/train_embeddings_bert.pkl'
# csv_file = '../MELD.Raw/train_sent_emo.csv'
# embeddings_final = 'unimodal_text_train_embeddings.pkl'

# pkl_file = '../embeddings/dev_embeddings_bert.pkl'
# csv_file = '../MELD.Raw/dev_sent_emo.csv'
# embeddings_final = 'unimodal_text_dev_embeddings.pkl'

pkl_file = '../embeddings/test_embeddings_bert.pkl'
csv_file = '../MELD.Raw/test_sent_emo.csv'
embeddings_final = 'unimodal_text_test_embeddings.pkl'
...
for row in csv_reader:
    if count == 0:
        ...
        if (row[5] + "_" + row[6]) not in missing:
            emotion = {'neutral': 0, 'surprise': 1, 'fear': 2, 'sadness': 3, 'joy': 4, 'disgust': 5, 'anger': 6}
            dev_ids[row[5] + '_' + row[6]] = {"dialogue_id": row[5], "utterance_id": row[6], "bert_embedding": dev_embeddings[row[0]], "emotion": emotion[row[3]]}
...
for key in dev_ids:
    ...
    if int(utt_id) > 1:
        try:
            new_embedding = torch.cat((dev_ids[dia_id + '_' + str(int(utt_id) - 2)]['bert_embedding'][0][-1], dev_ids[dia_id + '_' + str(int(utt_id) - 1)]['bert_embedding'][0][-1], dev_ids[key]['bert_embedding'][0][-1]))
            new_dev_ids[count] = {"dialogue_id": dia_id, "utterance_id": utt_id, "bert_embedding": new_embedding, "emotion": dev_ids[key]["emotion"]}
            count += 1
        except KeyError:
            pass
